...
void C_ConfigManager::setup()
{
	...
	for (auto i = 0; i < 8; i++)
	{
		...
		setup_item(&g_cfg.ragebot.weapon[i].baim_settings, 6, std::to_string(i) + crypt_str("Ragebot.body_aim_conditions"));
	...
	for (auto i = 0; i < 4; i++)
	{
		...
		setup_item(&g_cfg.antiaim.type[i].desync_range, 100, std::to_string(i) + crypt_str("Antiaim.desync_range"));
	...
	setup_item(&g_cfg.esp.world_color, Color::White, crypt_str("Esp.world_color"));
	setup_item(&g_cfg.esp.props_color, Color::White, crypt_str("Esp.props_color"));
