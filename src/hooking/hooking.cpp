...
#include "../../lib/sdk/RenderContext.h"
#include "../../lib/sdk/SoundInfo.h"
#include "../../lib/sdk/SoundEmitter.h"
#include "../../lib/sdk/StudioRender.h"
#include "../../lib/sdk/Surface.h"
#include "../../lib/sdk/UserCmd.h"
#include "../../lib/sdk/UserMessages.h"
...
bool deviceInit(IDirect3DDevice9* device) {
    ...
    bool ret = LoadTextureFromFile(hooks->fullModulePath.c_str(), &hooks->texture, &hooks->width, &hooks->height);
    IM_ASSERT(ret);
    return true;
}

bool LoadTextureFromFile(const char* pSrcFile, LPDIRECT3DTEXTURE9* ppTexture, int* pWidth, int* pHeight) {
    LPDIRECT3DTEXTURE9 tempTexture;
    HRESULT CreateTextureResult = D3DXCreateTextureFromFileA(g_pd3dDevice, pSrcFile, &tempTexture);
    if (CreateTextureResult != S_OK)
        return false;

    D3DSURFACE_DESC fileInfo;
    HRESULT GetDescResult = tempTexture->GetLevelDesc(0, &fileInfo);
    if (GetDescResult != S_OK)
        return false;

    *ppTexture = tempTexture;
    *pWidth = (int)fileInfo.Width;
    *pHeight = (int)fileInfo.Height;
    return true;
}
...
static HRESULT __stdcall present(IDirect3DDevice9* device, const RECT* src, const RECT* dest, HWND windowOverride, const RGNDATA* dirtyRegion) noexcept {
    ...
    [[maybe_unused]] static bool pDeviceInit { deviceInit(device) };
...
static void STDCALL_CONV drawModelExecute(LINUX_ARGS(void* thisptr, )
                                          void* ctx,
                                          void* state,
                                          const ModelRenderInfo & info,
                                          matrix3x4 * customBoneToWorld) noexcept {
...
static int STDCALL_CONV emitSound(LINUX_ARGS(void* thisptr, )
                                  void* filter,
                                  int entityIndex,
                                  int channel,
                                  const char* soundEntry,
                                  unsigned int soundEntryHash,
                                  const char* sample,
                                  float volume,
                                  int seed,
                                  int soundLevel,
                                  ...
                                  float soundtime,
                                  int speakerentity,
                                  void* soundParams) noexcept {
    Sound::modulateSound(soundEntry, entityIndex, volume);
    Misc::autoAccept(soundEntry);

    volume = std::clamp(volume, 0.0f, 1.0f);
    return hooks->sound.callOriginal<int, WIN32_LINUX(5, 6)>(filter,
                                                             entityIndex,
                                                             channel,
                                                             soundEntry,
                                                             soundEntryHash,
...
static bool STDCALL_CONV isPlayingDemo(LINUX_ARGS(void* thisptr)) noexcept {
    if (Misc::shouldRevealMoney() && RETURN_ADDRESS() == memory->demoOrHLTV
        && *reinterpret_cast<std::uintptr_t*>(FRAME_ADDRESS() + WIN32_LINUX(8, 24)) == memory->money)
        return true;
