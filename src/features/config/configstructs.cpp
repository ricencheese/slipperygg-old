...
void to_json(json &j, const Color4 &o, const Color4 &dummy) {
    if (o.color != dummy.color) {
        std::ostringstream s;
        s << '#' << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(o.color[0] * 255) << std::setw(2) << static_cast<int>(o.color[1] * 255)
          << std::setw(2) << static_cast<int>(o.color[2] * 255);
        j["Color"] = s.str();
        j["Alpha"] = o.color[3];
    }
...
void read(const json &j, const char* key, char* o, std::size_t size) noexcept {
    ...
    if (const auto &val = j[key]; val.is_string()) {
        strncpy_s(o, sizeof(o), val.get<std::string>().c_str(), size);
        o[size - 1] = '\0';
    }
}
