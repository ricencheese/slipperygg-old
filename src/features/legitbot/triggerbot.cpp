...
void Triggerbot::run(UserCmd* cmd) noexcept {
    if (!localPlayer || !localPlayer->isAlive() || localPlayer->nextAttack() > memory->globalVars->serverTime() || localPlayer->isDefusing()
        || localPlayer->waitForNoAttack())
        return;
    ...
    if (!config->legitbot.triggerbot[weaponIndex].enabled)
        weaponIndex = getWeaponClass(activeWeapon->itemDefinitionIndex());

    if (!config->legitbot.triggerbot[weaponIndex].enabled)
        weaponIndex = 0;

    const auto& cfg = config->legitbot.triggerbot[weaponIndex];
    ...
    if (now - lastContact < config->legitbot.triggerbot[weaponIndex].burstTime) {
    ...
    float damage = (activeWeapon->itemDefinitionIndex() != WeaponId::Taser ? HitGroup::getDamageMultiplier(trace.hitgroup) : 1.0f) * weaponData->damage
        * std::pow(weaponData->rangeModifier, trace.fraction * weaponData->range / 500.0f);

    if (float armorRatio { weaponData->armorRatio / 2.0f }; activeWeapon->itemDefinitionIndex() != WeaponId::Taser
...
void Triggerbot::updateInput() noexcept {
    keyPressed = !config->legitbot.triggerbotKey.isSet() || config->legitbot.triggerbotKey.isDown();
}
