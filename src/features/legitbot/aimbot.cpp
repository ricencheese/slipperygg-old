...
static float handleBulletPenetration(SurfaceData* enterSurfaceData,
    ...
    damage -= 11.25f / penetration / penetrationModifier + damage * damageModifier
        + (exitTrace.endpos - enterTrace.endpos).squareLength() / 24.0f / penetrationModifier;
...
void Aimbot::updateInput() noexcept {
    if (config->legitbot.aimbotKeyMode == 0)
        keyPressed = config->legitbot.aimbotKey.isDown();
    if (config->legitbot.aimbotKeyMode == 1 && config->legitbot.aimbotKey.isPressed())
...
void Aimbot::run(UserCmd* cmd) noexcept {
    ...
    if (!cfg.aimbot[weaponIndex].enabled)
        weaponIndex = weaponClass;

    if (!cfg.aimbot[weaponIndex].enabled)
        weaponIndex = 0;

    if (!cfg.aimbot[weaponIndex].betweenShots && activeWeapon->nextPrimaryAttack() > memory->globalVars->serverTime())
        return;

    if (!cfg.aimbot[weaponIndex].ignoreFlash && localPlayer->isFlashed())
        return;

    if (cfg.aimbotOnKey && !keyPressed)
        return;

    if (cfg.aimbot[weaponIndex].enabled
        && (cmd->buttons & UserCmd::IN_ATTACK || cfg.aimbot[weaponIndex].autoShot || cfg.aimbot[weaponIndex].aimlock)
        && activeWeapon->getInaccuracy() <= cfg.aimbot[weaponIndex].maxAimInaccuracy) {

        if (cfg.aimbot[weaponIndex].scopedOnly && activeWeapon->isSniperRifle() && !localPlayer->isScoped())
            return;

        auto bestFov = cfg.aimbot[weaponIndex].fov;
        Vector bestTarget {};
        ...
        for (int i = 1; i <= interfaces->engine->getMaxClients(); i++) {
            ...
            if (!entity || entity == localPlayer.get() || entity->isDormant() || !entity->isAlive()
                || !entity->isOtherEnemy(localPlayer.get()) && !cfg.aimbot[weaponIndex].friendlyFire || entity->gunGameImmunity())
                continue;

            for (auto bone : { 8, 4, 3, 7, 6, 5 }) {
                const auto bonePosition = entity->getBonePosition(cfg.aimbot[weaponIndex].bone > 1 ? 10 - cfg.aimbot[weaponIndex].bone : bone);
                ...
                if (!cfg.aimbot[weaponIndex].ignoreSmoke && memory->lineGoesThroughSmoke(localPlayerEyePosition, bonePosition, 1))
                    continue;

                if (!entity->isVisible(bonePosition) && (cfg.aimbot[weaponIndex].visibleOnly || !canScan(entity,
                    bonePosition,
                    activeWeapon->getWeaponData(),
                    cfg.aimbot[weaponIndex].killshot ?
                    entity->health() :
                    cfg.aimbot[weaponIndex].minDamage,
                    cfg.aimbot[weaponIndex].friendlyFire)))
                    continue;
                ...
                if (cfg.aimbot[weaponIndex].bone)
                    break;
            }
        }
        ...
        if (bestTarget.notNull()) {
            ...
            if (lastCommand == cmd->commandNumber - 1 && lastAngles.notNull() && cfg.aimbot[weaponIndex].silent)
            ...
            angle /= cfg.aimbot[weaponIndex].smooth;
            cmd->viewangles += angle;
            if (!cfg.aimbot[weaponIndex].silent)
                interfaces->engine->setViewAngles(cmd->viewangles);

            if (cfg.aimbot[weaponIndex].autoScope && activeWeapon->nextPrimaryAttack() <= memory->globalVars->serverTime() && activeWeapon->isSniperRifle()
                && !localPlayer->isScoped())
                cmd->buttons |= UserCmd::IN_ATTACK2;

            if (cfg.aimbot[weaponIndex].autoShot && activeWeapon->nextPrimaryAttack() <= memory->globalVars->serverTime() && !clamped
                && activeWeapon->getInaccuracy() <= cfg.aimbot[weaponIndex].maxShotInaccuracy)
                cmd->buttons |= UserCmd::IN_ATTACK;

            if (clamped)
                cmd->buttons &= ~UserCmd::IN_ATTACK;

            if (clamped || cfg.aimbot[weaponIndex].smooth > 1.0f)
