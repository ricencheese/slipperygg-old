...
const auto& cfg = config->legitbot.backtrack;

void Backtrack::update(FrameStage stage) noexcept {
    if (stage == FrameStage::RENDER_START) {
        if (!cfg.enabled || !localPlayer || !localPlayer->isAlive()) {
        ...
        for (int i = 1; i <= interfaces->engine->getMaxClients(); i++) {
            ...
            while (records[i].size() > 3 && records[i].size() > static_cast<size_t>(timeToTicks(static_cast<float>(cfg.timeLimit) / 1000.f)))
                records[i].pop_back();

            if (auto invalid = std::find_if(std::cbegin(records[i]), std::cend(records[i]), [](const Record& rec) { return !valid(rec.simulationTime); });
                invalid != std::cend(records[i]))
...
void Backtrack::run(UserCmd* cmd) noexcept {
    ...
    for (int i = 1; i <= interfaces->engine->getMaxClients(); i++) {
        ...
        auto angle = Aimbot::calculateRelativeAngle(localPlayerEyePosition, origin, cmd->viewangles + (cfg.recoilBasedFov ? aimPunch : Vector {}));
    ...
    if (bestTarget) {
        if (records[bestTargetIndex].size() <= 3
            || (!cfg.ignoreSmoke && memory->lineGoesThroughSmoke(localPlayer->getEyePosition(), bestTargetOrigin, 1)))
            return;
        ...
        for (size_t i = 0; i < records[bestTargetIndex].size(); i++) {
            ...
            auto angle =
                Aimbot::calculateRelativeAngle(localPlayerEyePosition, record.origin, cmd->viewangles + (cfg.recoilBasedFov ? aimPunch : Vector {}));
            auto fov = std::hypotf(angle.x, angle.y);
            if (fov < bestFov) {
                bestFov = fov;
                bestRecord = i;
...
bool Backtrack::valid(float simtime) noexcept {
    ...
    auto delta =
        std::clamp(network->getLatency(0) + network->getLatency(1) + getLerp(), 0.f, cvars.maxUnlag->getFloat()) - (memory->globalVars->serverTime() - simtime);
