...
#include "../../protobuf/protobufreader.h"
#include "../../utils/gamedata.h"
#include "../../utils/interfaces.h"

#include "../../../lib/imgui/imgui_stdlib.h"

#include "../../../lib/sdk/Client.h"
#include "../../../lib/sdk/ClientClass.h"
#include "../../../lib/sdk/ClientMode.h"
#include "../../../lib/sdk/ConVar.h"
#include "../../../lib/sdk/Cvar.h"
#include "../../../lib/sdk/Engine.h"
#include "../../../lib/sdk/EngineTrace.h"
...
#include "../../../lib/sdk/Localize.h"
#include "../../../lib/sdk/LocalPlayer.h"
#include "../../../lib/sdk/NetworkChannel.h"
#include "../../../lib/sdk/Panorama.h"
#include "../../../lib/sdk/PlayerResource.h"
#include "../../../lib/sdk/UserCmd.h"
#include "../../../lib/sdk/WeaponSystem.h"

auto& cfg = config->other.misc;

bool Misc::shouldRevealMoney() noexcept { return cfg.revealMoney; }
bool Misc::shouldRevealSuspect() noexcept { return cfg.revealSuspect; }
bool Misc::shouldDisableModelOcclusion() noexcept { return cfg.disableModelOcclusion; }
bool Misc::shouldFixBoneMatrix() noexcept { return cfg.fixBoneMatrix; }
bool Misc::isRadarHackOn() noexcept { return cfg.radarHack; }
bool Misc::isMenuKeyPressed() noexcept { return cfg.menuKey.isPressed(); }
float Misc::maxAngleDelta() noexcept { return cfg.maxAngleDelta; }
float Misc::aspectRatio() noexcept { return cfg.aspectratio; }

void Misc::edgejump(UserCmd* cmd) noexcept {
    if (!cfg.edgejump || !cfg.edgejumpkey.isDown())
        return;
...
void Misc::slowwalk(UserCmd* cmd) noexcept {
    if (!cfg.slowwalk || !cfg.slowwalkKey.isDown())
        return;
    ...
    // FIXME: (Fast Walk) | Temporary comment as I can't confirm this works yet. (Maybe offer as Beta feature)
    /*
    if (!cfg.slowwalk) {
        interfaces->engine->clientCmdUnrestricted("bind shift +speed");
    };
    if (!cfg.slowwalk || !cfg.slowwalkKey.isDown())
        return;
...
void Misc::updateClanTag(bool tagChanged) noexcept {
    ...
    if (cfg.clocktag) {
        ...
        struct tm* const localTime = nullptr;
    ...
    else if (cfg.customClanTag) {
        if (memory->globalVars->realtime - lastTime < 0.6f)
            return;

        if (cfg.animatedClanTag && !clanTag.empty() && cfg.tagAnimationType == 0) { //basic rotation animation
        ...
        if (cfg.animatedClanTag && cfg.tagAnimationType == 1) { //less basic txt file based animation
...
void Misc::spectatorList() noexcept {
    ...
    if (cfg.spectatorList.pos != ImVec2 {}) {
        ImGui::SetNextWindowPos(cfg.spectatorList.pos);
        cfg.spectatorList.pos = {};
    }

    if (cfg.spectatorList.size != ImVec2 {}) {
        ImGui::SetNextWindowSize(ImClamp(cfg.spectatorList.size, {}, ImGui::GetIO().DisplaySize));
        cfg.spectatorList.size = {};
    }

    ImGuiWindowFlags windowFlags = ImGuiWindowFlags_NoCollapse;
    if (!gui->isOpen())
        windowFlags |= ImGuiWindowFlags_NoInputs;
    if (cfg.spectatorList.noTitleBar)
...
void Misc::noscopeCrosshair(ImDrawList* drawList) noexcept {
    if (!cfg.noscopeCrosshair.asColorToggle().enabled)
        return;
    ...
    drawCrosshair(drawList, ImGui::GetIO().DisplaySize / 2, Helpers::calculateColor(cfg.noscopeCrosshair.asColorToggle().asColor4()));
}

void Misc::recoilCrosshair(ImDrawList* drawList) noexcept {
    if (!cfg.recoilCrosshair.asColorToggle().enabled)
        return;
    ...
    if (ImVec2 pos; Helpers::worldToScreenPixelAligned(localPlayerData.aimPunch, pos))
        drawCrosshair(drawList, pos, Helpers::calculateColor(cfg.recoilCrosshair.asColorToggle().asColor4()));
}
...
void Misc::watermark() noexcept {
    ...
    ImGuiWindowFlags windowFlags =
        ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysAutoResize;
    if (!gui->isOpen())
        windowFlags |= ImGuiWindowFlags_NoInputs;
...
void Misc::prepareRevolver(UserCmd* cmd) noexcept {
    ...
    if (cfg.prepareRevolver && localPlayer && (!cfg.prepareRevolverKey.isSet() || cfg.prepareRevolverKey.isDown())) {
        const auto activeWeapon = localPlayer->getActiveWeapon();
...
void Misc::drawBombTimer() noexcept {
    ...
    ss << "Bomb on " << (!plantedC4.bombsite ? 'A' : 'B') << " : " << std::fixed << std::showpoint << std::setprecision(3)
        << (std::max)(plantedC4.blowTime - memory->globalVars->currenttime, 0.0f) << " s";

    ImGui::textUnformattedCentered(ss.str().c_str());

    ImGui::PushStyleColor(ImGuiCol_PlotHistogram, Helpers::calculateColor(cfg.bombTimer.asColor3()));
    ...
    if (plantedC4.defuserHandle != -1) {
        ...
        if (plantedC4.defuserHandle == GameData::local().handle) {
            ...
            std::ostringstream ss;
            ss << defusingPlayer->name << " is defusing: " << std::fixed << std::showpoint << std::setprecision(3)
...
void Misc::quickReload(UserCmd* cmd) noexcept {
    if (cfg.quickReload) {
        ...
        if (auto activeWeapon { localPlayer->getActiveWeapon() }; activeWeapon && activeWeapon->isInReload()
            && activeWeapon->clip() == activeWeapon->getWeaponData()->maxClip) {
...
bool Misc::changeName(bool reconnect, const char* newName, float delay) noexcept {
    ...
    if (!exploitInitialized && interfaces->engine->isInGame()) {
        if (PlayerInfo playerInfo; localPlayer && interfaces->engine->getPlayerInfo(localPlayer->index(), playerInfo)
            && (!strcmp(playerInfo.name, "?empty") || !strcmp(playerInfo.name, "\n\xAD\xAD\xAD"))) {
...
void Misc::bunnyHop(UserCmd* cmd) noexcept {
    ...
    if (cfg.bunnyHop && !localPlayer->isOnGround() && localPlayer->moveType() != MoveType::LADDER && !wasLastTimeOnGround)
        cmd->buttons &= ~UserCmd::IN_JUMP;
...
void Misc::fakeBan(bool set) noexcept {
    ...
    if (shouldSet && interfaces->engine->isInGame() && changeName(false,
        std::string { "\x1\xB" }.append(std::string { static_cast<char>(cfg.banColor + 1) })
        .append(cfg.banText).append("\x1").c_str(),
        5.0f))
        shouldSet = false;
}
...
void Misc::nadePredict() noexcept {
    ...
    nadeVar->setValue(cfg.nadePredict);
}

void Misc::fixTabletSignal() noexcept {
    if (cfg.fixTabletSignal && localPlayer) {
...
void Misc::killMessage(GameEvent& event) noexcept {
    ...
    cmd += cfg.killMessageString;
    cmd += '"';
    interfaces->engine->clientCmdUnrestricted(cmd.c_str());

    // FIXME: TODO: Returns FAT Linker errors all-round so change this to the UI version of killsay (similar to clantag UI).
    /*
    if (doesFileExist("C:/slippery/killsay.txt")) {                         //been testing if doesfileexist() works
        ...
        interfaces->engine->clientCmdUnrestricted("say killsay file wasn't found :o");
    }
    */
}
...
void Misc::antiAfkKick(UserCmd* cmd) noexcept {
    if (cfg.antiAfkKick && cmd->commandNumber % 2)
        cmd->buttons |= 1 << 27;
}

void Misc::fixAnimationLOD(FrameStage stage) noexcept {
#ifdef _WIN32
    if (cfg.fixAnimationLOD && stage == FrameStage::RENDER_START) {
        if (!localPlayer)
            return;
...
void Misc::revealRanks(UserCmd* cmd) noexcept {
    if (cfg.revealRanks && cmd->buttons & UserCmd::IN_SCORE)
...
void Misc::moonwalk(UserCmd* cmd) noexcept {
    if (cfg.moonwalk && localPlayer && localPlayer->moveType() != MoveType::LADDER)
...
void Misc::playHitSound(GameEvent& event) noexcept {
    ...
    if (static_cast<std::size_t>(cfg.hitSound - 1) < hitSounds.size())
        interfaces->engine->clientCmdUnrestricted(hitSounds[cfg.hitSound - 1]);
    else if (cfg.hitSound == 5)
        interfaces->engine->clientCmdUnrestricted(("play " + cfg.customHitSound).c_str());
}
...
void Misc::killSound(GameEvent& event) noexcept {
    ...
    if (static_cast<std::size_t>(cfg.killSound - 1) < killSounds.size())
        interfaces->engine->clientCmdUnrestricted(killSounds[cfg.killSound - 1]);
    else if (cfg.killSound == 5)
        interfaces->engine->clientCmdUnrestricted(("play " + cfg.customKillSound).c_str());
}
    ...
    if (event) {
        ...
        case fnv::hash("item_purchase"):
        {
            if (const auto player = interfaces->entityList->getEntity(interfaces->engine->getPlayerForUserID(event->getInt("userid"))); player
                && localPlayer && localPlayer->isOtherEnemy(player)) {
        ...
        case fnv::hash("round_start"):freezeEnd = 0.0f;
            playerPurchases.clear();
            purchaseTotal.clear();
            totalCost = 0;
            break;
        case fnv::hash("round_freeze_end"):freezeEnd = memory->globalVars->realtime;
            break;
        }
    }
    else {
        if (!cfg.purchaseList.enabled)
            return;

        if (static const auto mp_buytime = interfaces->cvar->findVar("mp_buytime"); (!interfaces->engine->isInGame()
            || freezeEnd != 0.0f && memory->globalVars->realtime > freezeEnd + (!cfg.purchaseList.onlyDuringFreezeTime ? mp_buytime->getFloat() : 0.0f)
            || playerPurchases.empty() || purchaseTotal.empty()) && !gui->isOpen())
            return;
        ...
        if (!gui->isOpen())
            windowFlags |= ImGuiWindowFlags_NoInputs;
        if (cfg.purchaseList.noTitleBar)
            windowFlags |= ImGuiWindowFlags_NoTitleBar;
        ...
        if (cfg.purchaseList.mode == Config::Other::Misc::PurchaseList::Details) {
            GameData::Lock lock;

            for (const auto& [handle, purchases] : playerPurchases) {
                std::string s;
                s.reserve(std::accumulate(purchases.items.begin(),
                    purchases.items.end(),
                    0,
                    [](int length, const auto& p) { return length + p.first.length() + 2; }));
                ...
                if (const auto player = GameData::playerByHandle(handle)) {
                    if (cfg.purchaseList.showPrices)
        ...
        else if (cfg.purchaseList.mode == Config::Other::Misc::PurchaseList::Summary) {
            for (const auto& purchase : purchaseTotal)
                ImGui::TextWrapped("%d x %s", purchase.second, purchase.first.c_str());

            if (cfg.purchaseList.showPrices && totalCost > 0) {
                ImGui::Separator();
...
[[nodiscard]] static std::string generateReportString() {
    std::string report;
    if (cfg.reportbot.textAbuse)
        report += "textabuse,";
    if (cfg.reportbot.griefing)
        report += "grief,";
    if (cfg.reportbot.wallhack)
        report += "wallhack,";
    if (cfg.reportbot.aimbot)
        report += "aimbot,";
    if (cfg.reportbot.other)
        report += "speedhack,";
    return report;
}
...
[[nodiscard]] static std::vector<std::uint64_t> getXuidsOfCandidatesToBeReported() {
    ...
    for (int i = 1; i <= interfaces->engine->getMaxClients(); ++i) {
        ...
        if (cfg.reportbot.target != 2 && (localPlayer->isOtherEnemy(entity) ? cfg.reportbot.target != 0 : cfg.reportbot.target != 1))
            continue;
...
void Misc::runReportbot() noexcept {
    ...
    if (lastReportTime + cfg.reportbot.delay > memory->globalVars->realtime)
        return;

    if (reportbotRound >= cfg.reportbot.rounds)
        return;
...
void Misc::preserveKillfeed(bool roundStart) noexcept {
    if (!cfg.preserveKillfeed.enabled)
        return;
    ...
    for (int i = 0; i < childPanelCount; ++i) {
        ...
        if (child->hasClass("DeathNotice_Killer") && (!cfg.preserveKillfeed.onlyHeadshots || child->hasClass("DeathNoticeHeadShot")))
...
void Misc::voteRevealer(GameEvent& event) noexcept {
    ...
    memory->clientMode->getHudChat()->printf(0,
                                             " \x0C\u2022Osiris\u2022 %c%s\x01 voted %c%s\x01",
                                             isLocal ? '\x01' : color,
                                             isLocal ? "You" : entity->getPlayerName().c_str(),
                                             color,
                                             votedYes ? "Yes" : "No");
}
...
void Misc::onVoteStart(const void* data, int size) noexcept {
    ...
    memory->clientMode->getHudChat()->printf(0,
                                             " \x0C\u2022Osiris\u2022 %c%s\x01 call vote (\x06%s\x01)",
                                             isLocal ? '\x01' : '\x06',
                                             isLocal ? "You" : entity->getPlayerName().c_str(),
                                             voteName(voteType));
}
...
static void shadeVertsHSVColorGradientKeepAlpha(ImDrawList* draw_list,
                                                int vert_start_idx,
                                                int vert_end_idx,
                                                ImVec2 gradient_p0,
                                                ImVec2 gradient_p1,
...
void Misc::drawOffscreenEnemies(ImDrawList* drawList) noexcept {
    if (!cfg.offscreenEnemies.enabled)
        return;
    ...
    for (auto& player : GameData::players()) {
        ...
        const auto trianglePos = pos + ImVec2 { x, y } *(avatarRadius + (cfg.offscreenEnemies.healthBar.enabled ? 5 : 3));
        ...
        const auto color = Helpers::calculateColor(cfg.offscreenEnemies.asColor4());
        const auto healthBarColor =
            cfg.offscreenEnemies.healthBar.type == HealthBar::HealthBased ? Helpers::healthColor(std::clamp(player.health / 100.0f, 0.0f, 1.0f)) :
            Helpers::calculateColor(cfg.offscreenEnemies.healthBar.asColor4());
        ...
        ImGui::ShadeVertsLinearUV(drawList,
                                  vertStartIdx,
                                  vertEndIdx,
                                  pos - ImVec2 { avatarRadius, avatarRadius },
                                  pos + ImVec2 { avatarRadius, avatarRadius },
                                  { 0, 0 },
                                  { 1, 1 },
                                  true);
        ...
        if (cfg.offscreenEnemies.healthBar.enabled) {
            ...
            if (cfg.offscreenEnemies.healthBar.type == HealthBar::Gradient)
                shadeVertsHSVColorGradientKeepAlpha(drawList,
                                                    vertStartIdx,
                                                    vertEndIdx,
                                                    pos - ImVec2 { 0.0f, radius },
                                                    pos + ImVec2 { 0.0f, radius },
...
void Misc::updateEventListeners(bool forceRemove) noexcept {
    ...
    if (cfg.purchaseList.enabled && !listenerRegistered) {
        interfaces->gameEventManager->addListener(&listener, "item_purchase");
        listenerRegistered = true;
    }
    else if ((!cfg.purchaseList.enabled || forceRemove) && listenerRegistered) {
