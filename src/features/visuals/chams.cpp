...
#include "../../../lib/sdk/ModelRender.h"
#include "../../../lib/sdk/StudioRender.h"
#include "../../../lib/sdk/KeyValues.h"
...
static void initializeMaterials() noexcept {
    ...
    glow = interfaces->materialSystem
        ->createMaterial("glow", KeyValues::fromString("VertexLitGeneric", "$additive 1 $envmap models/effects/cube_white $envmapfresnel 1 $alpha .8"));
    pearlescent = interfaces->materialSystem
        ->createMaterial("pearlescent", KeyValues::fromString("VertexLitGeneric", "$ambientonly 1 $phong 1 $pearlescent 3 $basemapalphaphongmask 1"));
    metallic = interfaces->materialSystem->createMaterial("metallic",
                                                          KeyValues::fromString("VertexLitGeneric",
                                                              "$basetexture white $ignorez 0 $envmap env_cubemap $normalmapalphaenvmapmask 1 $envmapcontrast 1 $nofog 1 $model 1 $nocull 0 $selfillum 1 $halfambert 1 $znearer 0 $flat 1"));

    {
        const auto kv = KeyValues::fromString("VertexLitGeneric",
                                              "$envmap editor/cube_vertigo $envmapcontrast 1 $basetexture dev/zone_warning proxies { texturescroll { texturescrollvar $basetexturetransform texturescrollrate 0.6 texturescrollangle 90 } }");
    ...
    {
        const auto kv = KeyValues::fromString("VertexLitGeneric",
                                              "$baseTexture black $bumpmap effects/flat_normal $translucent 1 $envmap models/effects/crystal_cube_vertigo_hdr $envmapfresnel 0 $phong 1 $phongexponent 16 $phongboost 2");
    ...
    {
        const auto kv = KeyValues::fromString("VertexLitGeneric",
                                              "$baseTexture white $bumpmap effects/flat_normal $envmap editor/cube_vertigo $envmapfresnel .6 $phong 1 $phongboost 2 $phongexponent 8");
    ...
    {
        const auto kv = KeyValues::fromString("VertexLitGeneric",
                                              "$baseTexture white $bumpmap effects/flat_normal $envmap editor/cube_vertigo $envmapfresnel .6 $phong 1 $phongboost 6 $phongexponent 128 $phongdisablehalflambert 1");
    ...
    {
        const auto kv = KeyValues::fromString("VertexLitGeneric",
                                              "$baseTexture black $bumpmap models/inventory_items/trophy_majors/matte_metal_normal $additive 1 $envmap editor/cube_vertigo $envmapfresnel 1 $normalmapalphaenvmapmask 1 $phong 1 $phongboost 20 $phongexponent 3000 $phongdisablehalflambert 1");
...
bool Chams::render(void* ctx, void* state, const ModelRenderInfo& info, matrix3x4* customBoneToWorld) noexcept {
    if (config->visuals.visualsKey.isSet()) {
        if (!config->visuals.visualsKey.isToggled())
            return false;
    }
...
void Chams::renderPlayer(Entity* player) noexcept {
    ...
    if (const auto activeWeapon = player->getActiveWeapon(); activeWeapon && activeWeapon->getClientClass()->classId == ClassId::C4
        && activeWeapon->c4StartedArming()
        && std::ranges::any_of(config->visuals.chams["Planting"].materials, [](const Config::Visuals::Chams::Material& mat) { return mat.enabled; })) {
        applyChams(config->visuals.chams["Planting"].materials, health);
    }
    else if (player->isDefusing()
     && std::ranges::any_of(config->visuals.chams["Defusing"].materials, [](const Config::Visuals::Chams::Material& mat) { return mat.enabled; })) {
        applyChams(config->visuals.chams["Defusing"].materials, health);
    }
    else if (player == localPlayer.get()) {
        applyChams(config->visuals.chams["Local player"].materials, health);
    }
    else if (localPlayer->isOtherEnemy(player)) {
        applyChams(config->visuals.chams["Enemies"].materials, health);
        ...
        if (records && !records->empty() && Backtrack::valid(records->front().simulationTime)) {
            ...
            applyChams(config->visuals.chams["Backtrack"].materials, health, records->back().matrix);
            interfaces->studioRender->forcedMaterialOverride(nullptr);
        }
    }
    else {
        applyChams(config->visuals.chams["Allies"].materials, health);
    }
}
...
void Chams::renderWeapons() noexcept {
    ...
    applyChams(config->visuals.chams["Weapons"].materials, localPlayer->health());
}
...
void Chams::renderHands() noexcept {
    ...
    applyChams(config->visuals.chams["Hands"].materials, localPlayer->health());
}
...
void Chams::renderSleeves() noexcept {
    ...
    applyChams(config->visuals.chams["Sleeves"].materials, localPlayer->health());
}

void Chams::applyChams(const std::array<Config::Visuals::Chams::Material, 7>& chams, int health, const matrix3x4* customMatrix) noexcept {
