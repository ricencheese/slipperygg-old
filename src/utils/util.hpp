...
namespace util
{
	...
	static T* FindHudElement(const char* name)
	{
		...
		if (!pThis)
			pThis = *reinterpret_cast<DWORD**>(util::FindSignature(crypt_str("client.dll"), crypt_str("B9 ?? ?? ?? ?? E8 ?? ?? ?? ?? 8B 5D 08")) + 0x1);
	...
	static Type hook_manual(uintptr_t* vftable, uint32_t index, Type fnNew) 
	{
		...
		VirtualProtect((void*)(vftable + index * sizeof(Type)), sizeof(Type), PAGE_EXECUTE_READWRITE, &OldProtect); //-V2001
		vftable[index] = (uintptr_t)fnNew;
		VirtualProtect((void*)(vftable + index * sizeof(Type)), sizeof(Type), OldProtect, &OldProtect); //-V2001
