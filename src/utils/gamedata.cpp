...
#include "../../res/avatar_ct.h"
#include "../../res/avatar_tt.h"

#include "../../lib/sdk/ClassId.h"
...
#include "../../lib/sdk/ModelInfo.h"
#include "../../lib/sdk/ModelRender.h"
#include "../../lib/sdk/NetworkChannel.h"
#include "../../lib/sdk/PlantedC4.h"
#include "../../lib/sdk/PlayerResource.h"
#include "../../lib/sdk/Sound.h"
#include "../../lib/sdk/Steam.h"
#include "../../lib/sdk/UtlVector.h"
#include "../../lib/sdk/WeaponId.h"
#include "../../lib/sdk/WeaponData.h"
    ...
    for (int i = 1; i <= highestEntityIndex; ++i) {
        ...
        if (entity->isPlayer()) {
            ...
            if (entity->isWeapon()) {
                ...
                case ClassId::SnowballPile:entityData.emplace_back(entity);
                    break;
                case ClassId::LootCrate:lootCrateData.emplace_back(entity);
                    break;
                case ClassId::Inferno:infernoData.emplace_back(entity);
                    break;
                default:break;
                }
            }
...
void GameData::clearUnusedAvatars() noexcept {
    Lock lock;
    std::erase_if(playerAvatars,
                  [](const auto& pair) { return std::ranges::find(std::as_const(playerData), pair.first, &PlayerData::handle) == playerData.cend(); });
}
    ...
    if (const auto weaponInfo = entity->getWeaponData()) {
        group = [](WeaponType type, WeaponId weaponId) {
            ...
            default:
                ...
                case WeaponId::Shield:return "Other";
                default: return "All";
                }
            }
...
ObserverData::ObserverData(Entity* entity, Entity* obs, bool targetIsLocalPlayer) noexcept : playerHandle { entity->handle() }, targetHandle { obs->handle() },
