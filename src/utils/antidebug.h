#ifndef LWBASE_SRC_UTILS_ANTIDEBUG_H
#define LWBASE_SRC_UTILS_ANTIDEBUG_H
#include <Windows.h>
#include "sha-256.h"
#include "imports.h"
...
using NtSetInformationThreadFn = NTSTATUS(__stdcall *)(_In_ HANDLE, _In_ THREAD_INFORMATION_CLASS, _In_ PVOID, _In_
                                                       ULONG);
using NtQueryInformationProcessFn = NTSTATUS(__stdcall *)(_In_ HANDLE, _In_  unsigned int, _Out_ PVOID, _In_ ULONG,
                                                          _Out_ PULONG);
...
public:
    ...
    int is_debugging() {
        ...
        auto allocation = IFH(VirtualAlloc)(nullptr, system_info.dwPageSize, MEM_COMMIT | MEM_RESERVE,
        ...
        if (allocation) {
            ...
            *pMem = 0xC3;
            ...
            if (IFH(VirtualProtect)(allocation, system_info.dwPageSize, PAGE_EXECUTE_READWRITE | PAGE_GUARD,
                                    &old_protect)) {
                auto check = false;
        ...
        if (ntdll) {
            auto NtSetInformationThread = (NtSetInformationThreadFn) GetProcAddress(ntdll, crypt_str(
                    "NtSetInformationThread"));

            if (NtSetInformationThread)
                NtSetInformationThread(IFH(GetCurrentThread)(), (THREAD_INFORMATION_CLASS) 17, nullptr, 0);

            auto NtQueryInformationProcess = (NtQueryInformationProcessFn) GetProcAddress(ntdll, crypt_str(
        ...
        __try
        {
            ...
            if (IFH(FindWindow)(crypt_str("ID"), nullptr))
                return 28;
            if (IFH(FindWindow)(crypt_str("IDA: Quick start"), nullptr))
                return 29;
            if (IFH(FindWindow)(crypt_str("IDA - lwbase.dll"), nullptr))
                return 30;
        }
};
#endif
