...
//--------------------------------------------------------------------------------
void Visuals::Player::RenderSnapline()				//snapline render
{

	int screen_w, screen_h;										//this shit is fucking retarded, when you come close
	g_EngineClient->GetScreenSize(screen_w, screen_h);			//to the snapline target, it flies like 20 meters up
																//when you're far away, it goes under the floor like
	Render::Get().RenderLine(screen_w / 2.f, screen_h / 2.f,	//a fucking kilometer
		ctx.head_pos.x, ctx.head_pos.y, ctx.clr);				//https://gatoo.xyz/l/Gzvz0K3Qd/
		//ctx.feet_pos.x, ctx.feet_pos.y, ctx.clr);				//https://gatoo.xyz/l/LWCtLCj8I/
																//https://gato-host-is.cool/l/g_yhtjywH/ 
																//--note, snapline target is set to HEAD not FLOOR/SKY
}
//--------------------------------------------------------------------------------
void Visuals::RenderCrosshair()
{												//don't even bother using it, it's ugly, it just draws 2 x and y lines
/*	int w, h;									//use weapon_debug_spread_show 3 instead, look it up in hooks.cpp
