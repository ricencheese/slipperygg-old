...
namespace Hooks {
	...
	long __stdcall hkEndScene(IDirect3DDevice9* pDevice)
	{
		...
		static auto custom_fov = g_CVar->FindVar("fov_cs_debug");
		...
		static auto crosshair_cvar = g_CVar->FindVar("weapon_debug_spread_show");
		...
		static auto fog_start_sky = g_CVar->FindVar("fog_startskybox");
		static auto fog_end_sky = g_CVar->FindVar("fog_endskybox");
		static auto nightmode_amount = g_CVar->FindVar("fog_maxdensity");
		static auto nightmode_amount_skybox = g_CVar->FindVar("fog_maxdensityskybox");
		static auto nightmode_color = g_CVar->FindVar("fog_color");

		viewmodel_fov->m_fnChangeCallbacks.m_Size = 0;
		viewmodel_fov->SetValue(g_Options.viewmodel_fov);
		custom_fov->m_fnChangeCallbacks.m_Size = 0;
		custom_fov->SetValue(g_Options.custom_fov);
		...
		fog_override->SetValue(g_Options.misc_nightmode);					//nightmode(no idea if that's how people
		fog_start->SetValue(g_Options.misc_nightmode);						//usually do it but it works for me :) )
		fog_end->SetValue(g_Options.misc_nightmode);						//
		fog_start_sky->SetValue(g_Options.misc_nightmode);					//
		fog_end_sky->SetValue(g_Options.misc_nightmode);					//
		nightmode_amount->SetValue(g_Options.misc_nightmode_amount);		//
		nightmode_amount_skybox->SetValue(g_Options.misc_nightmode_amount);	//
		nightmode_color->SetValue(g_Options.color_nightmode);				//
		
		if (g_Options.esp_enabled && g_Options.esp_crosshair) {				//the better sniper crosshair
			crosshair_cvar->SetValue(3.f);
		}
		else { crosshair_cvar->SetValue(0.f); }
