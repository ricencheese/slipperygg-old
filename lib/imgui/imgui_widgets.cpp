...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, float power, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    if ((GetHoveredID() == id || g.ActiveId == id))
    {
        auto right_press = ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_RightArrow));
        auto right_down = ImGui::IsKeyDown(ImGui::GetKeyIndex(ImGuiKey_RightArrow));

        auto left_press = ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_LeftArrow));
        auto left_down = ImGui::IsKeyDown(ImGui::GetKeyIndex(ImGuiKey_LeftArrow));

        if (left_down || right_down)
        {
            if (data_type == ImGuiDataType_Float)
                *v += power * (left_down ? -1 : 1);
            else
                *v += power * (left_down ? -1 : 1);
        }
        ...
        {
            if (data_type == ImGuiDataType_Float)
                *v += (power / 3) * (left_down ? -1 : 1);
            else
                *v += ImFloor(power / 3) * (left_down ? -1 : 1);
        }
    }
...
bool ImGui::ColorEdit(const char* label, Color* newcolor, ImGuiColorEditFlags flags)
{
    ...
    float col[4] =
    {
        (float)(*newcolor).r() / 255.f,
        (float)(*newcolor).g() / 255.f,
        (float)(*newcolor).b() / 255.f,
        (float)(*newcolor).a() / 255.f
    };
