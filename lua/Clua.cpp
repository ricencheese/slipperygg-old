...
namespace ns_entitylist
{
	...
	{
		std::cout << player.value()->m_hActiveWeapon().Get()->get_name() << std::endl;
		if (!m_engine()->IsInGame())
...
namespace ns_http //new
{
	...
	{
		if (!g_cfg.scripts.alow_http)
		{
			c_lua::get().unload_script(get_current_script_id(s));
			eventlogs::get().add(crypt_str("Please alow http"), false);
			return "";
		}
		try
		{
	...
	std::string post(sol::this_state s, std::string& link, std::string& params)
	{
		...
		try
		{
			http::Request request(link);
			// send a post request
			const http::Response response = request.send(crypt_str("POST"), params, {
				crypt_str("Content-Type: application/x-www-form-urlencoded")
				});
			return std::string(response.body.begin(), response.body.end());
		}
		catch (const std::exception& e)
